...
def initial_setting(source, out, weights):
    ...
    return [out, weights, imgsz, device, half,
            model, classify, save_img, names, colors,
            ORIGINAL_R, ORIGINAL_C, DANGER_ZONE_MATRIX_R, DANGER_ZONE_MATRIX_C
            ]


def get_image_return_detected_object_list(setting, source):
    out, weights, imgsz, device, half,\
    model, classify, save_img, names, colors, \
    ORIGINAL_R, ORIGINAL_C, DANGER_ZONE_MATRIX_R, DANGER_ZONE_MATRIX_C = setting

    img = torch.zeros((1, 3, imgsz, imgsz), device=device)  # init img
    model(img.half() if half else img) if device.type != 'cpu' else None  # run once
...
def get_tracking_object_list_danger_list(ORIGINAL_R, ORIGINAL_C,
    ...
    print(tracking_object_list_danger_list)
    return tracking_object_list_danger_list


def draw_box_and_lines_on_image(setting, tracking_object_list, tracking_object_list_danger_list, im0):
    out, weights, imgsz, device, half, \
    model, classify, save_img, names, colors, \
    ORIGINAL_R, ORIGINAL_C, DANGER_ZONE_MATRIX_R, DANGER_ZONE_MATRIX_C = setting
...
def one_photo_input_all_in_one(setting, source, weights, tracking_object_list, danger_zone_matrix):
    if not setting:
        setting = initial_setting(source, 'inference/output', weights)
    ORIGINAL_R, ORIGINAL_C, DANGER_ZONE_MATRIX_R, DANGER_ZONE_MATRIX_C = setting[10:14]
    detected_object_list = get_image_return_detected_object_list(setting, source)
    tracking_object_list = append_detect_object_to_tracking_object_list(detected_object_list, tracking_object_list)
    tracking_object_list_danger_list = is_tracking_object_list_dangerous(ORIGINAL_R,
                                                                         ORIGINAL_C,
                                                                         DANGER_ZONE_MATRIX_R,
                                                                         DANGER_ZONE_MATRIX_C,
                                                                         tracking_object_list,
                                                                         danger_zone_matrix)
    im0 = imread(source)
    draw_box_and_lines_on_image(setting, tracking_object_list, tracking_object_list_danger_list, im0)
    return tracking_object_list
